<template>
    ...
    <div id="address-container" style="margin: auto">
        ...
        <form class="address" @submit.prevent="orderHandler">
            ...
            <button @click="getLocation" class="btn btn-danger" style="background-color: rgb(231, 114, 114);" type="button">Get Current Location</button>
    ...
    methods: {
        async orderHandler(){
            ...
            try {
                ...
                if(resData.status==="success"){
                    this.$store.dispatch("setTotalCartItems");
        ...
        async getLocation(){
            navigator.geolocation.getCurrentPosition(async (position)=>{
                const res = await fetch(`https://api.mapbox.com/geocoding/v5/mapbox.places/${position.coords.longitude},${position.coords.latitude}.json?access_token=pk.eyJ1IjoiZ3VwdHN3YXlhbSIsImEiOiJjazhmN3dueGowMThlM29vZnhxdGdjMWpkIn0.mITs9dATeHdGHcr19Vf7TQ`);
                const resData = await res.json();
                // console.log(resData);
                const currentLocation = resData.features[1].place_name.split(",");
                this.formData.address = currentLocation.slice(0, currentLocation.length - 3).join(",");
                this.formData.city = currentLocation[currentLocation.length-3];
                this.formData.state = currentLocation[currentLocation.length - 2];
            });
        }
    }
}
